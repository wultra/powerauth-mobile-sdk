...
public class PowerAuthConfigurationBuilderTest {
    ...
    public void testSdkConfigurationValidation() throws Exception {
        String sdkConfig = "";
        String expectedErrorMessage = "Failed to read data version from SDK config";
        ...
        expectedErrorMessage = "SDK config is missing required ECDSA_P256 master server public key";
        assertSdkConfiguration(sdkConfig, expectedErrorMessage);
        ...
        expectedErrorMessage = getAlgorithmForTest() > PowerAuthAlgorithm.LEGACY_P256
                ? "SDK config is missing required ECDSA_P384 master server public key"
                : null;
        assertSdkConfiguration(sdkConfig, expectedErrorMessage);
        ...
        if (getAlgorithmForTest() == PowerAuthAlgorithm.EC_P384_ML_L3) {
            expectedErrorMessage = "SDK config is missing required MLDSA_65 master server public key";
        } else if (getAlgorithmForTest() == PowerAuthAlgorithm.EC_P384_ML_L5) {
            expectedErrorMessage = "SDK config is missing required MLDSA_87 master server public key";
        } else {
        ...
        expectedErrorMessage = getAlgorithmForTest() < PowerAuthAlgorithm.EC_P384_ML_L5
                ? null
                : "SDK config is missing required MLDSA_87 master server public key";
        assertSdkConfiguration(sdkConfig, expectedErrorMessage);
