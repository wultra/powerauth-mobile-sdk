...
import io.getlime.security.powerauth.integration.tests.TestParameters;
...
public class PowerAuthConfigurationBuilderTest {
    ...
    @Parameterized.Parameter() public String alg;
    ...
    /**
     * Test of validation error messages.
     */
    @Test
    public void testSdkConfigurationValidation() throws Exception {
        String sdkConfig = "";
        String expectedErrorMessage = "powerAuth::PowerAuthException: Invalid SDK configuration string";
        assertSdkConfiguration(sdkConfig, expectedErrorMessage);

        // SDK config without keys
        sdkConfig = "ARDDj6EB6iAUtNmNxKM/BsbaEEs5bP+yVmyjfhQDoox3LDwA";
        expectedErrorMessage = "powerAuth::PowerAuthException: Configuration is missing KEY_MASTER_P256_PUBLIC key required for " + alg + " algorithm";
        assertSdkConfiguration(sdkConfig, expectedErrorMessage);
        ...
        expectedErrorMessage = getAlgorithmForTest() > PowerAuthAlgorithm.LEGACY_P256
                ? "powerAuth::PowerAuthException: Configuration is missing KEY_MASTER_ECDSA_P384_PUBLIC key required for " + alg + " algorithm"
                : null;
        assertSdkConfiguration(sdkConfig, expectedErrorMessage);
        ...
        if (getAlgorithmForTest() == PowerAuthAlgorithm.EC_P384_ML_L3) {
            expectedErrorMessage = "powerAuth::PowerAuthException: Configuration is missing KEY_MASTER_MLDSA65_PUBLIC key required for " + alg + " algorithm";
        } else if (getAlgorithmForTest() == PowerAuthAlgorithm.EC_P384_ML_L5) {
            expectedErrorMessage = "powerAuth::PowerAuthException: Configuration is missing KEY_MASTER_MLDSA87_PUBLIC key required for " + alg + " algorithm";
        } else {
        ...
        sdkConfig = "ARDDj6EB6iAUtNmNxKM/BsbaEEs5bP+yVmyjfhQDoox3LDwDAUEEQQ7CWNKAi0EgCfOvd/srfqz4" +
                ...
                "WSFtbALuBxtGb9vBdw==";
        expectedErrorMessage = getAlgorithmForTest() < PowerAuthAlgorithm.EC_P384_ML_L5
                ? null
                : "powerAuth::PowerAuthException: Configuration is missing KEY_MASTER_MLDSA87_PUBLIC key required for " + alg + " algorithm";
        assertSdkConfiguration(sdkConfig, expectedErrorMessage);

        // This SDK configuration contains all keys
        assertSdkConfiguration(sdkConfiguration, null);
    }

    private void assertSdkConfiguration(final String sdkConfiguration, final String errorMessage) throws PowerAuthErrorException {
        final PowerAuthConfiguration.Builder configuration = new PowerAuthConfiguration.Builder(
                null, "http://wultra.com", sdkConfiguration)
                .algorithm(getAlgorithmForTest());

        if (errorMessage != null) {
            final var exception = assertThrows(PowerAuthErrorException.class, configuration::build);
            assertEquals(PowerAuthErrorCodes.WRONG_PARAMETER, exception.getPowerAuthErrorCode());
            if (exception.getCause() instanceof CoreException) {
                // Embedded, compare CoreException
                assertEquals(errorMessage, exception.getCause().getMessage());
            } else {
                // Not embedded exception, compare main reason
                assertEquals(errorMessage, exception.getMessage());
            }
        } else {
            assertNotNull(configuration.build());
        }
    }
}
