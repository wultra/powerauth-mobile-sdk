...
public class PowerAuthSDKBuilderTest {
    ...
    private String goodSdkConfigV3 =
            "ARDDj6EB6iAUtNmNxKM/BsbaEEs5bP+yVmyjfhQDoox3LDwBAUEEQQ7CWNKAi0EgCfOvd/srfqz4" +
            "oqhTMLwsT4r7sPLRfqICRw9cCMs/Uoo/F2rIz+KKEcBxbnH9bMk8Ju3K1wmjbA==";
    private String goodSdkConfigV4 =
    ...
    void prepareDefaultConfiguration() throws Exception {
        powerAuthConfiguration = buildConfigurationFor(PowerAuthAlgorithm.DEFAULT);
    }

    PowerAuthConfiguration buildConfigurationFor(@PowerAuthAlgorithm int algorithm) throws Exception {
        final String sdkConfig;
        if (algorithm == PowerAuthAlgorithm.LEGACY_P256) {
            sdkConfig = goodSdkConfigV3;
        } else {
            sdkConfig = goodSdkConfigV4;
        }
    ...
    @Test
    public void testAlgorithmSelection() throws Exception {
        @PowerAuthAlgorithm int[] algorithms = {
                PowerAuthAlgorithm.LEGACY_P256,
                PowerAuthAlgorithm.EC_P384,
                PowerAuthAlgorithm.EC_P384_ML_L3,
                PowerAuthAlgorithm.EC_P384_ML_L5,
        };
        for (int algorithm : algorithms) {
            PowerAuthConfiguration configuration = buildConfigurationFor(algorithm);
            PowerAuthSDK powerAuthSDK = new PowerAuthSDK.Builder(configuration)
                    .build(androidContext);
            assertEquals(algorithm, powerAuthSDK.getCurrentAlgorithm());
        }
    }
