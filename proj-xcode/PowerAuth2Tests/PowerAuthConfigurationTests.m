...
- (void) testSdkConfigurationValidation
{
    for (int algorithm = PowerAuthAlgorithm_LEGACY_P256; algorithm <= PowerAuthAlgorithm_EC_P384_ML_L5; ++algorithm) {
        NSString* sdkConfig = @"";
        NSString* expectedErrorMessage = @"Failed to read data version from SDK config";
                       ...
                       algorithm:(PowerAuthAlgorithm)algorithm
{
    ...
    PowerAuthValidationResult* result = [config validateConfigurationWithDetail];
    XCTAssertEqual(result.isValid, errorMessage == nil, @"Test for algorithm=%ld", (long)algorithm);
    XCTAssertEqualObjects(errorMessage, result.errorReason, @"Test for algorithm=%ld", (long)algorithm);
}
