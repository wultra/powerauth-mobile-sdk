...
@interface PowerAuthCorePassTests : XCTestCase
@end

@implementation PowerAuthCorePassTests
...
- (void) testImmutable
{
    PowerAuthCorePass * p1 = [PowerAuthCorePass passwordWithString:@"HelloWorld"];
    XCTAssertEqual(10, p1.length);
    XCTAssertEqualObjects(@"HelloWorld", [self extractStringFromPassword:p1]);
    
    PowerAuthCorePass * p2 = [PowerAuthCorePass passwordWithString:@"HelloWorld"];
    XCTAssertTrue([p1 isEqualToPassword:p2]);
    XCTAssertEqualObjects(p1, p2);
    
    DATA_BYTES(p3data, { 1, 2, 3, 4, 5, 6, 7 });
    PowerAuthCorePass * p3 = [PowerAuthCorePass passwordWithData:p3data];
...
{
    PowerAuthCoreMutablePass * p1 = [PowerAuthCoreMutablePass mutablePassword];
    ...
    XCTAssertEqualObjects([PowerAuthCorePass passwordWithData:expectedBytes1], p1);
    
    XCTAssertTrue([p1 removeCharacterAtIndex:0]);
    DATA_BYTES(expectedBytes2, { 1, 2, 3, 4});
    XCTAssertEqualObjects([PowerAuthCorePass passwordWithData:expectedBytes2], p1);
    
    XCTAssertTrue([p1 removeCharacterAtIndex:1]);
    DATA_BYTES(expectedBytes3, { 1, 3, 4});
    XCTAssertEqualObjects([PowerAuthCorePass passwordWithData:expectedBytes3], p1);
    
    XCTAssertTrue([p1 removeCharacterAtIndex:2]);
    DATA_BYTES(expectedBytes4, { 1, 3 });
    XCTAssertEqualObjects([PowerAuthCorePass passwordWithData:expectedBytes4], p1);
...
- (void) testPasswordEqual
{
    ...
    PowerAuthCorePass * p1 = [PowerAuthCorePass passwordWithString:@"fixed"];
    PowerAuthCorePass * p2 = [PowerAuthCorePass passwordWithData:p2data];
    PowerAuthCoreMutablePass * p3 = [PowerAuthCoreMutablePass mutablePassword];
    [p3 addCharacter:'f'];
    [p3 addCharacter:'i'];
    [p3 addCharacter:'x'];
    [p3 addCharacter:'e'];
...
{
    PowerAuthCorePass * p1 = [PowerAuthCorePass passwordWithString:@"fixed"];
    PowerAuthCorePass * p2 = [PowerAuthCorePass passwordWithString:@"strin"];
    PowerAuthCorePass * p3 = [PowerAuthCorePass passwordWithString:@"string"];
    PowerAuthCorePass * p4 = [PowerAuthCorePass passwordWithString:@"stri"];
    PowerAuthCorePass * p5 = [PowerAuthCoreMutablePass mutablePassword];
...
- (void) testPasswordCopyAndSecureClear
{
    ...
    PowerAuthCorePass * p1copy = [p1 copyToImmutable];
    PowerAuthCorePass * p2copy = [p2 copyToImmutable];
    PowerAuthCorePass * p3copy = [p3 copyToImmutable];
...
- (NSString*) extractStringFromPassword:(PowerAuthCorePass*)password
{
...
- (NSData*) extractBytesFromPassword:(PowerAuthCorePass*)password
{
