...
function USAGE
{
    ...
    echo "WARNING:"
    echo ""
    echo "  This script builds old and unsupported version of SDK."
    echo "  The result is xcframework that is used for protocol"
    echo "  migration tests. Do not use it for production builds."
    echo ""
    echo "commands are:"
    echo ""
...
function BUILD_LIB
{
    ...
    XCODE_SCHEME_IOS="PA2_Release"
    XCODE_TARGET_IOS="PowerAuth2"
    OUT_FW=${LIB_NAME}
...
function CLEAN_COMMAND
{
	...
	xcodebuild clean -project "${XCODE_PROJECT}" ${QUIET}
}
