...
{
    auto reader = utils::DataReader(cc7::Base64::decode(config), true);
    ...
    cc7::ByteRange p256key, p384key, mldsa64key;
    while (keys_count-- > 0) {
        cc7::byte key_id;
    ...
    if (p256key.empty() || p384key.empty() || mldsa64key.empty()) {
        return false;
    }
    // Finally, convert loaded values into setup structure
    applicationKey = app_key.base64();
    applicationSecret = app_secret.base64();
    legacyMasterServerPublicKey = p256key;
    ecdsaMasterServerPublicKey = p384key;
    mldsaMasterServerPublicKey = mldsa64key;
    return true;
}
...
std::string SessionSetup::saveConfiguration() const
{
    ...
    writer.writeData(cc7::Base64::decode(applicationKey));
    writer.writeData(cc7::Base64::decode(applicationSecret));
    writer.writeCount(3);
    writer.writeByte(P256_KEY_ID);
    writer.writeData(legacyMasterServerPublicKey);
    writer.writeByte(P384_KEY_ID);
    writer.writeData(ecdsaMasterServerPublicKey);
    writer.writeByte(MLDSA65_KEY_ID);
    writer.writeData(mldsaMasterServerPublicKey);
...
std::string Version_GetMaxSupportedHttpProtocolVersion(Version protocol_version)
{
    ...
    switch (protocol_version) {
        ...
        case Version_V4: return protocol::PA_VERSION_V4;
        default: break;
    }
    CC7_ASSERT(false, "Invalid protocol version");
    return protocol::PA_VERSION_V4;
}
